...
function habitLanesMarkup(cycle){
  ...
  const header=`<div class="habit-lane-header"><div><span>Action</span><small>Current streak</small><em class="habit-lane-thought">Small steps, repeated daily, shape a stronger life.</em></div><div class="habit-lane-days">${dates.map(date=>{const statuses=DASHBOARD_ACTIONS.map(action=>dashboardActionStatus(action,date)),eligible=statuses.filter(status=>!["not-scheduled","upcoming"].includes(status)).length,completed=statuses.filter(status=>status==="completed").length,value=eligible?Math.round(completed/eligible*100):0;return`<div class="habit-lane-date ${date===today?"today":""}"><i class="day-column-orb" style="--value:${value}">${isFuture(date)?"—":`${value}%`}</i><strong>${displayDate(date,{weekday:"short"})}</strong><span>${parseDate(date).getDate()}</span></div>`;}).join("")}</div></div>`;
